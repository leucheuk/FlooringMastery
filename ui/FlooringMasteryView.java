  ...
  public Order getNewOrderInfo(Order editOrder,  List<Tax> txLst, List<Product> prdctLst) {
    ...
    if (editmode){
        orderNumber = editOrder.getorderNumber();
            ...
            do {
                try {
                       orderNumber = io.readString("Please enter a new order number (enter = "+nextOrderNum+" )");
                       if(orderNumber == null || orderNumber.isEmpty()) {
                          orderNumber = String.valueOf(nextOrderNum);
...
//    currentOrder.setlaborCostPerSquareFoot(laborCostPerSquareFoot);
  ...
  public void displayOrderList(List<Order> orderList) {
    if(orderList.isEmpty()){
     ...
     orderList.stream().sorted(Comparator.comparing(Order::getorderNumber , (s1, s2) -> {
...
public String getdateChoice() {
  //  io.readString("Please enter the order date (MM/dd/yyyy): ");
      Boolean isValid = false;
      String orderDate=null;
           //  orderDate = io.readString("Please enter Order Date ");
                 do {
   ...
   return orderDate.substring(0, 2) + orderDate.substring(3, 5) + orderDate.substring(6, 10);     
}
