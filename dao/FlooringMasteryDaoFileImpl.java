...
import static com.sherlock.flooringmastery.controller.FlooringMasteryController.nextOrderNum;
...
public class FlooringMasteryDaoFileImpl implements FlooringMasteryDao {
    ...
    private final String FLOORING_FILE_DIR ; 
    
 //   public static int nextOrderNum = 1;
    
    public FlooringMasteryDaoFileImpl(){
           FLOORING_FILE_DIR = PROJECT_DIR + "\\Orders\\"; ;
    }
    
    public FlooringMasteryDaoFileImpl(String floorTextFolder){
           FLOORING_FILE_DIR = PROJECT_DIR + floorTextFolder ;
    }
 ...
 //           System.out.println(currentldAsText);
    ...
    IntStream istream = orders.values().stream().mapToInt((p) -> Integer.valueOf(p.getorderNumber()));
    nextOrderNum =(orders.isEmpty()? 1 : 1+ istream.max().getAsInt() );    
   // System.out.println("nextOrderNum is : #"+ nextOrderNum);
 ...
 //       List<File> orderFiles = listAllOrderFiles();
        ...
        Map<String, List<Order>> orderListSorted
                = orderList.stream().collect(Collectors.groupingBy((p) -> p.getOrderDate()));
        
        Set<String> keys = orderListSorted.keySet();
        
        for (String k : keys) {
    //        System.out.println(k); 
            try {
                  out = new PrintWriter(new FileWriter(FLOORING_FILE_DIR+"Orders_"
                           +k+".txt"));
                } catch (IOException e) {
                  throw new FlooringMasteryPersistenceException(
 ...
 /*
             orderListSorted.get(k).stream().sorted(Comparator
                     .comparing(Order::getorderNumber , (s1, s2) -> {
                        return valueOf(s1).compareTo(valueOf(s2));}))
                     .forEach((p) ->  {
                 out.println(marshallOrder(p));
                 out.flush();});
   */         
            List<Order> orderListGrouped = orderListSorted.get(k);
            for (Order currentOrder : orderListGrouped) {
...
//                    System.out.println("File " + listOfFiles[i].getName());
    ...
    public void deleteAllOrderFiles  () throws FlooringMasteryPersistenceException {
...
//                        }              
   //                Path sourceFile = Paths.get(FLOORING_FILE_DIR + listOfFiles[i].getName());
